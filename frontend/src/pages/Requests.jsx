    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-600">
          Gérez les demandes clients et leurs documents
        </p>
      </motion.div>
                  ...
                  rows.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-gray-600">
                        {row.client?.raison_sociale ?? row.client?.name ?? "—"}
                      </td>
                      <td className="px-6 py-4">
                        <Badge color={STATUS_COLORS[row.status] || "sky"}>
                          {row.status}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-gray-600">
...
function RequestDetails({ id, onStatus }) {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file) => {
      ...
      const { data } = await api.post(`/requests/${id}/files`, form, {
  ...
  const deleteFileMutation = useMutation({
    mutationFn: async (fileId) =>
      (await api.delete(`/requests/${id}/files/${fileId}`)).data,
    onSuccess: () => qc.invalidateQueries({ queryKey: ["request", id] }),
  });
  const addMessageMutation = useMutation({
    mutationFn: async (message_html) =>
      (await api.post(`/requests/${id}/messages`, { message_html })).data,
    onSuccess: () => {
      setComposer("");
      qc.invalidateQueries({ queryKey: ["request", id] });
    },
  ...
  const remindMutation = useMutation({
    mutationFn: async (note) =>
      (await api.post(`/requests/${id}/remind`, { note })).data,
    onSuccess: () => {
      setRelanceNote("");
      qc.invalidateQueries({ queryKey: ["request", id] });
    },
  ...
  const [relanceNote, setRelanceNote] = React.useState("");
  ...
  if (!data)
    return (
      <div className="text-gray-500 text-center py-8">Demande non trouvée</div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-gray-50 to-gray-100">
        ...
        <RequirePermission
          perm="requests.manage"
          fallback={<Badge color="gray">{data.status}</Badge>}
        >
          <Select
            value={data.status}
            ...
            {(data.request_messages ?? []).map((m, i) => (
              ...
              >
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: m.message_html }}
                />
              </motion.div>
            ))}
          </AnimatePresence>
          ...
          <RequirePermission perm="requests.manage" fallback={null}>
            <div className="mt-3 border-t pt-3">
              ...
              <textarea
                ...
                placeholder="Message (HTML autorisé)"
              ></textarea>
              <div className="mt-2 flex justify-end">
                <Button
                  onClick={() =>
                    composer.trim() && addMessageMutation.mutate(composer)
                  }
            ...
            {(data.request_files ?? []).map((f, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <RequirePermission perm="requests.manage" fallback={null}>
                    <button
                      className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-red-500 to-red-600 text-white text-xs"
      ...
      <RequirePermission perm="requests.manage" fallback={null}>
        <div className="p-4 rounded-xl bg-amber-50 border-2 border-amber-200 space-y-2">
          <div className="font-semibold text-amber-800">Relancer le client</div>
          <input
          ...
          <div className="flex justify-end">
            <Button
              ...
              onClick={() => remindMutation.mutate(relanceNote)}
            >
              Relancer
            </Button>
          </div>
      ...
      <ConfirmDialog
        ...
        description={
          confirmFile ? `Supprimer "${confirmFile.original_name}" ?` : ""
        }
        onCancel={() => setConfirmFile(null)}
        onConfirm={() => {
          if (confirmFile) {
            deleteFileMutation.mutate(confirmFile.id);
            setConfirmFile(null);
          }
        }}
      />
