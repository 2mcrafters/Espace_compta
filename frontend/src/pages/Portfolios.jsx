  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={
          editing?.id ? "Modifier le portefeuille" : "Nouveau portefeuille"
        }
        footer={
      ...
      <ConfirmDialog
        ...
        description={
          confirmDelete
            ? `Confirmez la suppression de "${confirmDelete.name}". Les rattachements clients seront détachés.`
            : ""
        }
...
function PortfolioCollaboratorsModal({ portfolio, onClose }) {
  ...
  const { data: collabs = [] } = useQuery({
    queryKey: ["portfolio-collabs", portfolio?.id],
    queryFn: async () =>
      (await api.get(`/portfolios/${portfolio.id}/collaborators`)).data,
    enabled: open,
  });

  const attachMutation = useMutation({
    mutationFn: async (user_id) =>
      (await api.post(`/portfolios/${portfolio.id}/collaborators`, { user_id }))
        .data,
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["portfolio-collabs", portfolio?.id] }),
  });
  const detachMutation = useMutation({
    mutationFn: async (user_id) =>
      (await api.delete(`/portfolios/${portfolio.id}/collaborators/${user_id}`))
        .data,
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["portfolio-collabs", portfolio?.id] }),
  });
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={
        portfolio ? `Collaborateurs • ${portfolio.name}` : "Collaborateurs"
      }
      footer={
        <div className="flex gap-2">
          <Button
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <h4 className="font-semibold mb-2 text-gray-900 dark:text-gray-100">
            Collaborateurs du portefeuille
          </h4>
          <ul className="divide-y">
            {collabs.map((u) => (
              <li key={u.id} className="py-2 flex items-center justify-between">
      ...
      <ConfirmDialog
        ...
        description={
          confirmDetach
            ? `Retirer ${confirmDetach.name} de ${portfolio?.name} ?`
            : ""
        }
